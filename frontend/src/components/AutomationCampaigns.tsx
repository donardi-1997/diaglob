...
const initial = (): AutomationCampaignPayload => ({
  ...
  cooldown_days: 7, channel: "whatsapp", message_template: "Hola {{customer.name}},",
});
...
export default function AutomationCampaigns({ canWrite, storeId }: Props) {
  ...
  return <section className="automation-campaigns">
    ...
    {open && <div className="modal-backdrop"><div className="modal automation-wizard" role="dialog" aria-modal="true"><h2>{t("autoNewCampaign")}</h2><div className="wizard-steps">{[1, 2, 3, 4].map((number) => <button key={number} className={step === number ? "active" : ""} onClick={() => setStep(number)}>{number}. {t(`autoWizard${number}`)}</button>)}</div>
      {step === 1 && <div className="automation-wizard-body"><label>{t("autoFormName")}<input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label>{t("autoAudienceType")}<select value={form.audience_type} onChange={(event) => setForm({ ...form, audience_type: event.target.value as "dynamic" | "fixed" })}><option value="dynamic">{t("autoDynamicAudience")}</option><option value="fixed">{t("autoFixedAudience")}</option></select></label>{form.audience_type === "dynamic" ? <><label>{t("autoSegment")}<select onChange={(event) => updateFilters("segment", event.target.value ? [event.target.value] : [])}><option value="">-</option>{["new", "interested", "high_intent", "buyer", "repeat_buyer", "vip", "inactive"].map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label>{t("autoHealth")}<select onChange={(event) => updateFilters("health", event.target.value ? [event.target.value] : [])}><option value="">-</option><option value="active">active</option><option value="at_risk">at_risk</option><option value="inactive">inactive</option></select></label></> : <label>{t("autoCustomerIds")}<input placeholder="1, 2, 3" onChange={(event) => setForm({ ...form, member_ids: event.target.value.split(",").map(Number).filter(Number.isInteger) })} /></label>}<button className="secondary-button" onClick={() => void previewAudience()}>{t("autoPreview")}</button>{preview && <p>{t("autoEligible")}: {preview.eligible_count}</p>}</div>}
      ...
      {step === 3 && <div className="automation-wizard-body"><label>{t("autoMessage")}<textarea value={form.message_template} onChange={(event) => setForm({ ...form, message_template: event.target.value })} /></label><p>{t("autoVariables")}: {"{{customer.name}}, {{store.name}}, {{customer.segment}}, {{customer.health}}"}</p></div>}
